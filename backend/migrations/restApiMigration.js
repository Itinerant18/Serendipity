// Create Shop With Us Tables via REST API
// Uses individual REST calls to create tables one by one
...
async function createPaymentMethods() {
    ...
    const { data, error } = await supabase
        ...
        .insert([
            { name: 'Razorpay', gateway: 'razorpay', method_type: 'credit_card', is_active: true, processing_time: 'instant', transaction_fee_percent: 2.00, supported_countries: '["IN"]' },
            { name: 'UPI', gateway: 'razorpay', method_type: 'upi', is_active: true, processing_time: 'instant', transaction_fee_percent: 0.00, supported_countries: '["IN"]' },
            { name: 'Credit Card', gateway: 'razorpay', method_type: 'credit_card', is_active: true, processing_time: 'instant', transaction_fee_percent: 2.50, supported_countries: '["IN"]' },
            { name: 'Debit Card', gateway: 'razorpay', method_type: 'debit_card', is_active: true, processing_time: 'instant', transaction_fee_percent: 2.50, supported_countries: '["IN"]' },
            { name: 'Net Banking', gateway: 'razorpay', method_type: 'net_banking', is_active: true, processing_time: '2-3_days', transaction_fee_percent: 0.00, supported_countries: '["IN"]' }
        }]);
...
async function createRegionalSettings() {
    ...
    const { data, error } = await supabase
        ...
        .insert([{
            ...
            currency: 'INR',
            tax_inclusive: true,
            special_holidays: JSON.stringify(["2026-01-26", "2026-03-14"])
        }]);
...
async function insertSamplePolicyDetails() {
    console.log('Creating sample policy details...');
    
    const policyCategories = await supabase
        .from('policy_categories')
        .select('id', 'name')
        .eq('is_active', true);

    const [shippingCategory, paymentCategory, returnsCategory] = await Promise.all([
        policyCategories.then(cats => cats.find(cat => cat.name === 'shipping')),
        policyCategories.then(cats => cats.find(cat => cat.name === 'payment')),
        policyCategories.then(cats => cats.find(cat => cat.name === 'returns'))
    ]);
    ...
    const { data, error } = await supabase
        ...
        .insert([
            ...
            },
            {
                ...
                description: '100% secure payment gateway with multi-layer security. Accept all major payment methods including UPI, credit cards, and net banking.',
                icon: 'fa-lock',
                content: JSON.stringify({
                    methods: ["razorpay", "upi", "credit_card", "debit_card"],
                    "2fa": true,
                    "pci_compliant": true
                }),
                is_featured: true,
            ...
            },
            {
                ...
                description: 'Return any item within 30 days of delivery. Free pickup service available from your home. Instant refund processing within 24 hours of pickup.',
                icon: 'fa-rotate-left',
                content: JSON.stringify({
                    window: 30,
                    free_pickup: true,
    ...
    if (error) {
        console.error('Error inserting sample policy details:', error);
        throw error;
    }
...
async function runMigration() {
    try {
        ...
        await insertSamplePolicyDetails();
