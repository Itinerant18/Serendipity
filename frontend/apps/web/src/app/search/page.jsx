...
const ActiveFilterChip = ({ label, onRemove }) => (
  <motion.button
    ...
    onClick={onRemove}
    className="flex items-center gap-2 px-4 py-1.5 bg-pink-500 text-white border-4 border-black text-sm font-bold hover:bg-orange-500 hover:border-white hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[6px_6px_0_#000000] transition-transform duration-100"
  >
    <span>{label}</span>
...
export default function SearchPage() {
  ...
  }, [products, searchQuery, selectedCategories, selectedSubcategories, priceRange, selectedBrands, minRating, sortBy, inStockOnly, onSaleOnly]);

  // Reset page when filters change
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedCategories, selectedSubcategories, priceRange, selectedBrands, minRating, sortBy, inStockOnly, onSaleOnly, searchQuery]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen pt-32 pb-12 px-4 flex justify-center items-center bg-white border-8 border-black">
        <div className="flex flex-col items-center gap-6">
          ...
          <p className="text-black font-brutalist text-lg bg-yellow-400 px-4 py-2 border-4 border-black">SEARCHING PRODUCTS...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F0F9FF]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Search Header */}
        <div className="mb-8">
          <div className="bg-black text-white border-4 border-white p-6 inline-block mb-4">
            ...
            {filteredProducts.length > 0 && (
              <p className="text-sm text-yellow-400 mt-1">{filteredProducts.length} items found</p>
            )}
          </div>
        </div>
            ...
            <AnimatePresence mode="popLayout">
              {paginatedProducts.length > 0 ? (
                    ...
                    {paginatedProducts.map((product) => (
                      ...
                      >
                        <ProductCard
                          ...
                          className="h-full border-4 border-black bg-white shadow-[8px_8px_0_#000000] hover:bg-pink-500 hover:text-white hover:border-white hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[10px_10px_0_#000000] transition-transform duration-100 overflow-hidden"
                        />
                      </motion.div>
                    ))}
                  </motion.div>
                          ...
                          if (
                            ...
                            return (
                              <button
                                ...
                                className={`w-10 h-10 flex items-center justify-center font-bold border-4 border-black transition-colors ${currentPage === page
                            ...
                            return <span key={page} className="w-8 flex items-center justify-center text-gray-600 font-bold">...</span>;
                          }
                          return null;
                        })}
                      </div>
                ...
                <motion.div
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="flex flex-col items-center justify-center py-28 bg-white text-black border-4 border-black"
                >
                  ...
                  <h3 className="text-3xl font-brutalist text-black mb-2 bg-yellow-400 px-4 border-4 border-black">NO MATCHES FOUND</h3>
                  <p className="text-black max-w-md px-4 font-bold text-center">
                    {searchQuery ? `No products match "${searchQuery}"` : "TRY ADJUSTING FILTERS"}
                  </p>
                  <button
                    onClick={clearFilters}
                    className="mt-6 px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white border-4 border-black font-bold hover:border-white hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[8px_8px_0_#000000] transition-transform duration-100"
                  >
                    Reset Filters
                  </button>
                </motion.div>
